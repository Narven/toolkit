...
// randomStringSource as the source of the string
...
func (t *Tools) UploadFiles(r *http.Request, uploadDir string, rename ...bool) ([]*UploadedFile, error) {
	...
	for _, fHeaders := range r.MultipartForm.File {
		for _, hdr := range fHeaders {
			uploadedFiles, err := func(upFiles []*UploadedFile) ([]*UploadedFile, error) {
				...
				// instead of check the file type via the extension we take a look at the
				// first 512 bytes
				buff := make([]byte, 512)
...
// DownloadStaticFile it downloads a file, and tries to force the browser to avoid displaying it
...
func (t *Tools) ReadJSON(w http.ResponseWriter, r *http.Request, data interface{}) error {
	...
	// check the limit of the body request
	r.Body = http.MaxBytesReader(w, r.Body, int64(maxBytes))

	dec := json.NewDecoder(r.Body)

	// do we allow unkown fields?
	...
	// check if there are multiple JSON objects in the request
